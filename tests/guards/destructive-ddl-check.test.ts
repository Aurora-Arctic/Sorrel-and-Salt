...
// Pins the three ways the destructive-DDL check goes quietly wrong: scanning
// migrations already on the base branch, scanning `meta/*.json` beside the
// `.sql`, and reading `DROP` as only COLUMN and TABLE. Each case fails with its
// guard removed; the script's own --self-test sees two fixture files only.
...
describe('the branch diff', () => {
  ...
  beforeAll(() => {
    ...
    // Generated by `db:generate` and not yet staged.
  ...
  it('scans the whole repository only when asked to, under --all', () => {
    ...
    // 0002 is acknowledged, which is why --all is an audit rather than a gate.
...
describe('the acknowledgement sidecar', () => {
   ...
   * Keyed by SIDECAR path through `sidecarPath`, the real reader's derivation:
   * a map keyed by migration path would match nothing and leave several cases
  ...
  // Two files, one acknowledged — the case a single PR-body line let through.
  ...
  // Anchored at the start of a line, so prose around it does not match.
...
// The half the injected reader above cannot cover, and the half CI runs.
...
// Retired, not OR-ed with the sidecar: an OR would keep one line blessing every
// file. With the body gone the check reads nothing from GitHub, which is what
// lets `make act-check CHECK=destructive-ddl` prove the scan rather than the wiring.
...
// The sidecars are load-bearing: the repository's own `--all` audit is green
// *because* of them.
describe('the migrations in this repository', () => {
  it('has a sidecar for every migration carrying destructive DDL', () => {
    ...
    // Empty, and the loop below proves nothing.
