...
| `npm run lint` / `format:check` / `typecheck` (`npm run pre-commit`)       | The pre-commit checks — test-free by decision (MB.38); the mechanical guards in `src/test/` run in CI's `vitest` job                                                                 |
...
| `npm run db:seed` / `db:reset` (`make db-*`)                               | `db:seed` seeds `minimal` (M1.21) via `tsx`; `standard` is implemented (M1.22) but only reachable from `seed(db, …)` until M1.24 wires scenario selection; `demo` throws until M1.23 |
