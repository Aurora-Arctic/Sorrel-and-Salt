...
| `npm run db:seed` / `db:reset` (`make db-*`)                               | `db:seed` seeds `minimal` (M1.21) via `tsx`; `standard` (M1.22) and `demo` (M1.23) are implemented but only reachable from `seed(db, …)` until M1.24 wires scenario selection |
