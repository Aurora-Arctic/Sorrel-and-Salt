...
- Each Vitest **`db`-project** worker clones `sorrel_test_${VITEST_POOL_ID}` from the baked `sorrel_template` — the pool _slot_, never `VITEST_WORKER_ID`, which counts test files rather than workers and so runs past the set of clones that exist (MB.14); the `unit` project gets no such rewrite and sees the plain `sorrel` database, so anything touching Postgres belongs in `db` — which is to say under `tests/db/`, since the split is a path glob. Playwright uses `sorrel_e2e`. Do not wrap tests in a rolled-back transaction — `withAudit` opens its own, and `SET LOCAL` would leak one test user's identity into the next assertion.
