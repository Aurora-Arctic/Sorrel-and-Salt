...
**3. All writes go through `withAudit(session, fn)`; all workspace-scoped reads go through `withViewer(session, fn)`.**
`withAudit` opens the transaction, injects the audit ids from the _session_ (never from a request body), and publishes the acting user to the database as a transaction-local GUC — `select set_config('app.current_user_id', $1, true)`, not a literal `SET LOCAL`, which accepts no bind parameters and would mean interpolating a user id into SQL text. Every table carries the six-column `...auditColumns` spread, join tables included. `withViewer` is its read-only counterpart and publishes the same GUC, because a `LOCAL` setting exists only inside a transaction and the read path opens none of its own — without it every RLS policy on a read returns zero rows or raises on the `::uuid` cast. It is re-entrant, so services can establish it while the GraphQL route establishes one outer transaction per operation alongside the DataLoaders. The two never nest: a mutation writes inside `withAudit`, then resolves its response inside `withViewer`, and `withAudit` throws if a viewer transaction is already ambient.
...
`assertMembership(userId, workspaceId, minRole)` in the service, and an RLS policy reading the acting user through `app.current_user_id()` in the database. They look redundant; that is the point. M6.5 and M10.3 prove the database layer holds with the service check stubbed out.

**The application connects as a role that owns nothing.** Postgres skips a table's policies for that table's owner, so an app connected as the owner gets a second layer that parses, migrates, and does nothing. `sorrel` owns the schema, runs migrations and seeds, and holds `BYPASSRLS`; `sorrel_app` owns no table, holds `NOBYPASSRLS`, and is what the application connects as — derived in `connection.ts` from whatever `DATABASE_URL` the platform supplies plus `DATABASE_APP_ROLE`/`DATABASE_APP_PASSWORD`, so it is correct on a Neon preview branch whose host is minted at deploy time. Tables are `FORCE`d as well as `ENABLE`d, and a startup assertion refuses any role that is superuser, holds `rolbypassrls`, or owns an application table. Policies read identity only through `app.current_user_id()`; `app.is_member()` is `security definer` because a `workspace_members` policy that subqueries `workspace_members` is infinite recursion. A test that asserts RLS refused something must also assert why it could have succeeded — a green RLS test connected as the owner proves nothing. See [`mb.24-rls-role-split.md`](claude-docs/design-decisions/mb.24-rls-role-split.md).
...
- **Admins curate the compendium, global categories, and the ingredient form vocabulary — and nothing else.** A site admin has no access to any workspace's ingredients or grimoire — asserted by test (M6.6).
...
- **Tests never touch Neon.** Neon is deployment-only. Local Postgres 18 in Docker everywhere else — SQLite cannot run RLS, triggers, `pg_trgm`, or array columns, which are exactly what needs testing.
