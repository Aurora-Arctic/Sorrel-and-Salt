...
| `make act-check`                                                           | Run a check locally via `act`; `act-check CHECK=<leg>` picks a `checks.yml` leg, `make act-test` chains four, `make act-overlay` runs its second job (MB.42) and wants a clean tree                                                                    |
