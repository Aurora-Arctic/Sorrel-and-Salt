...
- **The Neon integration injects the table-owning role.** Its `DATABASE_URL` is `neondb_owner`, and a per-PR preview branch mints its endpoint host at deploy time, so no connection string can be pre-set for one. Left alone that would run the app as a table owner, where RLS policies are silently skipped. The app therefore derives its own credentials from whatever `DATABASE_URL` it is given, swapping only user and password, and asserts at startup that it is not connected as an owner (§8).
...
Application layer: every workspace-scoped call passes `assertMembership(userId, workspaceId, minRole)` in the service. A write runs inside `withAudit()`, so it can't reach the database without both a membership check and audit stamping; a read runs inside `withViewer()`, its read-only counterpart. Both publish the acting user as the transaction-local `app.current_user_id` GUC — which is what makes the second layer possible on reads as well as writes, since a `LOCAL` setting exists only inside a transaction and the read path opens none of its own.

Database layer: RLS policies reading that GUC back.

```sql
CREATE POLICY inventory_items_workspace_access ON inventory_items
  USING (app.is_member(workspace_id));
```

`app.is_member(uuid, workspace_role)` is a `security definer` function owned by the migration role, and `app.current_user_id()` beneath it returns `nullif(current_setting('app.current_user_id', true), '')::uuid`. Neither is decoration. A policy on `workspace_members` written as a subquery against `workspace_members` is infinite policy recursion, which Postgres rejects outright; and returning `NULL` rather than raising on an unset GUC means a misconfiguration denies instead of 500ing. Both also put the role hierarchy in one place instead of one copy per policy.

**The application does not own the tables it queries.** Postgres exempts a table's owner from that table's policies, so an application connected as the owner gets policies that parse, migrate, and do nothing. `sorrel` owns the schema, runs migrations and seeds, and holds `BYPASSRLS`; `sorrel_app` owns nothing, holds `NOBYPASSRLS`, and is what the application connects as. Tables are `FORCE`d as well as `ENABLE`d, so even a connection that turns out to be some _other_ table-owning role — Neon's `neondb_owner`, say — is still filtered. Three independent mechanisms, because ownership is invisible in a connection string: a startup assertion refuses to run as a role that is superuser, holds `rolbypassrls`, or owns an application table.

Belt and braces is warranted: an application bug here leaks one person's grimoire to another. The same two-layer pattern covers spell `visibility` — the service decides who may read a spell, and an RLS policy independently admits a `private` spell only to its author, along with its rows in `spell_ingredients` and `spell_categories`. Note that "workspace-scoped" is not the same as "has a `workspace_id` column": those two join tables reach their workspace through `spells`, and a guard that infers the set from a column name silently exempts the tables holding what a spell is made of.

Reasoning and rejected alternatives: [`mb.24-rls-role-split.md`](design-decisions/mb.24-rls-role-split.md).
