...
**`categories`** — `id`, `name`, `slug`, `color`, `description`, `groupId`, + audit. Global only, admin-curated. Suggestions in v2. Seed list in §6.
...
Each category carries `name`, `slug`, `color`, `description`, and `groupId`; each group carries `name`, `slug`, `colorDark`, `colorLight` and `description`. The table above is grouped for reading; the app lists groups alphabetically by `name`.
