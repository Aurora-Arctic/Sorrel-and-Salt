...
**The site is invite-gated.** Signing in creates an account with `canCreateWorkspace = false` and no workspace. Creation rights are granted by accepting a workspace invitation or by an admin (`grantWorkspaceCreation`), and persist once held. The first user matching the bootstrap env email is promoted to `admin` on first sign-in. Every admin after the first is granted by an existing admin, and the primary admin cannot be revoked (§5, M2.9).
...
| Route                         | Page                                                                                                                                                   |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/`                           | Public entry page: what the site is, that it is invite-only, and the way in — `/sign-in` for a visitor, `/coven` for someone signed in (MB.57)         |
| `/coven`                      | Post-sign-in landing: into their workspace if they have one; the create form if they hold creation rights; otherwise a plain "invite-only" explanation |
...
tests/acceptance/
  ...
  08-email-and-admin.test.ts # stories 58-61
```
...
- Invitation delivery — by email, accepted only by the verified holder of the address, viewer/member only; copy-link stands until MB.61's follow-ups bring delivery, and they correct §7's `url` and M7.3–M7.5 (M2.9), §5
- Dedupe — fuzzy warn in v1, merge in v2
- Categories — global admin-curated, 63 seeded, §6
- Granting admin — an admin grants admin, or invites one by email once verification exists (story 62), every change goes to a ledger, and the primary admin set by env var (promoted by Google or Discord only) cannot be revoked; §5, [`m2.9-granting-admin.md`](design-decisions/m2.9-granting-admin.md)
