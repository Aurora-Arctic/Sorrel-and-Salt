...
**`spell_ingredients`** — `spellId`, `ingredientId`, `quantity`, `unit`, `layerOrder`, `note`, + audit stamps. Composite primary key on `(spellId, ingredientId)`, no `deleted_at`, hard-deleted. (`note` here is a short free-text line on one ingredient's role in the jar, unrelated to the deferred notes subsystem.)
