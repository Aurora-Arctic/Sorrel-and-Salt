...
| `transcripts/<subsystem>.md`        | **An append-only transcript** — how a subsystem reached its shape. Newest entry last, never rewritten. Open waves only.             |
| `design-decisions/<mN.n>-<slug>.md` | A decision record — what was decided, why, and what it rules out. Open waves only.                                                  |
| `archive/wave-<n>/`                 | **Write-once.** Closed waves' transcripts and decision records, plus text cut from a live doc. See [`archive/`](archive/README.md). |
...
- A **transcript** is written as work lands: one entry per task or session,
...
- At the end of a wave, the `MW.<n>` compression pass trims the summaries down
  to what is still true, then **moves** that wave's transcripts and decision
  records into `archive/wave-<n>/`. Extraction comes first: nothing may end up
  living only in the archive.
...
- A second pass over a wave whose filenames are already taken writes a
