...
*Story:* As a developer, I want a running Next.js 15 App Router project with strict TypeScript so that every later task builds on the framework the design doc chose.
...
*Story:* As a developer, I want the same lint rules as resume-2026 so that code style is consistent across my repos and CI can gate on it.
...
*Story:* As a developer, I want formatting enforced before commit so that no PR review is spent on whitespace.
...
*Story:* As a developer, I want the familiar make targets so that I run the same commands in both repos.
...
*Story:* As a developer, I want the minimum shared Sass so that styling can start without importing a design that is going to change anyway.
...
*Story:* As a user, I want the app to look like a considered thing rather than a default so that it feels appropriate to what it holds.
...
*Story:* As a user, I want categories and safety warnings to be visually distinguishable so that I can scan a page without reading every label.
...
*Story:* As a developer, I want shared mixins for the three new UI primitives so that modals, chips and badges are consistent everywhere they appear.
...
*Story:* As a developer, I want the documentation convention in place from commit one so that subsystem docs accumulate instead of being retrofitted.
...
*Story:* As a developer, I want my existing agent skills available in this repo so that the assistant follows the same conventions here without being re-taught them.
...
*Story:* As a developer, I want a small image so that pulls, CI cold starts and rebuilds stay fast.
...
*Story:* As a developer, I want a working compose file so that `make docker-up` gives me the app without local Node version juggling.
...
*Story:* As a developer, I want a local Postgres so that tests exercise RLS, triggers and pg_trgm rather than a substitute that cannot.
...
*Story:* As a developer, I want the devcontainer to come up with a database attached so that a fresh clone is productive immediately.
...
*Story:* As a developer, I want the reusable composite actions available so that each check workflow stays a thin wrapper.
...
*Story:* As a reviewer, I want style and type checks on every PR so that review time goes to logic.
...
*Story:* As a developer, I want build and dependency-audit checks so that a broken build or a known vulnerability never reaches staging.
...
*Story:* As a developer, I want one database image shared by CI and local development so that every environment starts from an identical Postgres in seconds.
...
*Story:* As a developer, I want CI and local development to use the same database image so that a data-layer bug reproduces identically in both.
...
*Story:* As a reviewer, I want one gate aggregating every check so that a PR's mergeability is a single signal.
...
*Story:* As a developer, I want a merge queue so that main and staging never receive an untested merge combination.
...
*Story:* As a developer, I want the Gitflow rules enforced so that branch discipline does not depend on memory.
...
*Story:* As a developer, I want to run workflows locally so that CI debugging does not require pushing commits.
...
*Story:* As a developer, I want the application container image published so that CI and the devcontainer pull a prebuilt image instead of rebuilding.
...
*Story:* As a developer, I want only main and staging to deploy so that build minutes and function meters are not spent on every feature branch.
...
*Story:* As a developer, I want a stable staging URL so that OAuth callbacks and manual testing have a fixed target.
...
*Story:* As a developer, I want every secret named and scoped up front so that no environment is discovered to be missing one mid-deploy.
...
*Story:* As a developer, I want the whole pipeline proven end to end before any feature work so that deploy problems surface while the app is trivial.
...
*Story:* As a developer, I want deployment databases provisioned so that staging and production have isolated data from the first migration.
...
*Story:* As a developer, I want a single typed database client so that every query path goes through one configured connection.
...
*Story:* As a developer, I want migration commands wired before any schema exists so that every table arrives through the same mechanism.
...
*Story:* As a developer, I want migrations applied automatically on merge so that a deploy never runs against an older schema.
...
*Story:* As a developer, I want the schema to stay compatible with the previous app version so that reverting a bad release is a deploy rollback and never a database rollback.
...
*Story:* As an operator, I want a known-good point to return to so that a migration that corrupts data is recoverable rather than merely regrettable.
...
*Story:* As a developer, I want two test projects so that pure logic runs fast in jsdom while data-layer tests get a real Postgres connection.
...
*Story:* As a developer, I want the existing test setup carried over so that RTL and jsdom quirks are already solved.
...
*Story:* As a developer, I want isolated databases per Vitest worker so that parallel data-layer tests cannot interfere, without maintaining a test-database harness to get it.
...
*Story:* As a developer, I want component tests to mock the GraphQL endpoint so that they never require a running server.
...
*Story:* As a developer, I want e2e runs against a production build and a real database so that they catch what component tests cannot.
...
*Story:* As a user relying on assistive technology, I want the app to meet accessibility standards so that I can use every page and modal.
...
*Story:* As a developer, I want e2e coverage reported so that the two suites' contributions are visible separately.
...
*Story:* As a developer, I want test jobs skipped for docs-only changes so that trivial PRs are not slowed by the full suite.
...
*Story:* As an owner, I want every record to carry who created and last changed it so that a shared workspace has an accountable history.
...
*Story:* As a developer, I want one write path so that audit stamping cannot be skipped by a new call site.
...
*Story:* As a reviewer, I want the choke point enforced mechanically so that I do not have to catch violations by eye.
...
*Story:* As an owner, I want updated_at to be correct even after a manual database fix so that the audit trail cannot be quietly bypassed.
...
*Story:* As a developer, I want the current user available to the database so that RLS policies can enforce access independently of application code.
...
*Story:* As a user, I want deleted records to stay recoverable but invisible so that a mistaken delete is not permanent and does not clutter my lists.
...
*Story:* As a developer, I want one seed module used by Docker, Vitest and Playwright so that a bug reproduces identically in all three.
...
*Story:* As a developer, I want a fixed cast of users and workspaces so that authorization tests read clearly and consistently.
...
*Story:* As a developer, I want realistic seeded content so that manual testing and screenshots show the app as a user would see it.
...
*Story:* As a developer, I want a one-command reset so that a corrupted local database is never a blocker.
...
*Story:* As a developer, I want factories with sensible defaults so that tests state only what they are actually testing.
...
*Story:* As a developer, I want a uniform way to act as a fixture user so that authorization tests are one line rather than five.
...
*Story:* As a developer, I want the shared image to carry a ready-to-clone template so that no environment spends time migrating or seeding before it can run a test.
...
*Story:* As a product owner, I want a live checklist of which user stories pass so that progress is measured against requirements rather than coverage percentage.
...
*Stories 1–2 — As a developer, I want the account stories expressed as failing tests before implementation, so that done is measured against the specification rather than my own reading of it.*
...
*Story:* As a developer, I want auth running in-process so that there is no extra service, no extra deploy and no extra cost.
...
*Story 18 precondition — As the site owner, I want admin rights to exist on a real account, so that someone can curate the compendium before anyone depends on it.*
...
*Story 1 — As a new user, I want to sign in with Google, so that I don't have to manage another password.*
...
*Story 1 — As a new user, I want to sign in with GitHub, so that I don't have to manage another password.*
...
*Story 1 — As a new user, I want one sign-in page offering Google and GitHub, so that I can get in without managing another password.*
...
*Story:* As a user, I want unauthenticated access blocked so that my grimoire is not reachable by anyone with the URL.
...
*Story 2 — As a newly signed-in user, I want to be told plainly what I can do next, so that an empty account does not look like a broken one.*
...
*Story:* As the site owner, I want a considered plan for granting admin rights so that promoting a second admin does not mean editing an environment variable and redeploying.
...
*Story:* As a developer, I want a single GraphQL endpoint in-process so that the API adds no hosting cost and no separate deploy.
...
*Story:* As a developer, I want a typed code-first schema so that a resolver returning the wrong shape is a compile error.
...
*Story:* As an operator, I want expensive or probing queries rejected everywhere the app is reachable so that a client cannot burn my compute budget or map the schema, and so the protections are exercised before they matter.
...
*Story:* As a reviewer, I want schema changes to be visible in the diff so that a contract change is never silent.
...
*Story:* As a developer, I want generated client types to match the schema so that the client cannot drift from the server.
...
*Story:* As an operator, I want every list bounded by one shared rule so that no query can ask for the whole table, and no future list query can forget to say so.
...
*Story:* As a developer, I want a light client so that the bundle does not carry a second normalized cache duplicating TanStack Query.
...
*Story:* As a developer, I want server components to read through services directly so that a server-rendered page does not pay to serialize a GraphQL round trip to itself.
...
*Story:* As a reviewer, I want the two access paths enforced mechanically so that the boundary does not erode one convenient import at a time.
...
*Story:* As a user, I want my own profile available to the client so that the shell can render my identity and memberships.
...
*Story:* As a user, I want one ingredient table covering the shared reference and my own additions so that spells point at a single kind of thing.
...
*Story:* As a user on a phone, I want categories grouped so that selecting from 52 chips is manageable.
...
*Story:* As a user, I want a rich category vocabulary on day one so that I am not building the taxonomy myself before I can use the app.
...
*Story 22 — As a workspace member, I want ingredients to carry several categories, so that I can find things that are both protective and cleansing.*
...
*Story:* As a developer, I want one validation definition shared by client and server so that the two cannot disagree about what is valid.
...
*Story 16 precondition — As a developer, I want trigram matching available in the database, so that near-duplicate names can be detected before they multiply.*
...
*Story 16 — As a workspace member, I want to be warned when a name resembles something that already exists, so that I don't end up with three spellings of mugwort.*
...
*Story:* As an operator, I want related data batched so that a 50-ingredient page does not fire 101 queries and spend compute on nothing.
...
*Stories 17–18 — As a developer, I want the admin stories expressed as failing tests before implementation, so that the tool everything else is entered through is verified as it is built.*
...
*Story 17 — As a workspace member, I want to be prevented from editing compendium entries, so that shared reference data stays trustworthy for everyone.*
...
*Story 25 — As a workspace member, I want deletions to be recoverable, so that a mistake costs a request for help rather than my data.*
...
*Story 18 — As a site admin, I want an admin area only I can reach, so that I can curate shared data without exposing the surface to anyone else.*
...
*Story 18 — As a site admin, I want to add, edit and soft-delete compendium entries, so that the shared reference can grow without a deploy.*
...
*Story 18 — As a site admin, I want to add, edit and soft-delete categories, so that the taxonomy can change without a migration.*
...
*Stories 18 and 19 — As a workspace owner, I want admin rights to cover shared reference data and nothing else, so that an admin cannot reach into my workspace.*
...
*Story:* As a site admin, I want to approve someone who has no invitation, so that a person starting a coven of their own can get in without knowing an existing user.
...
*Stories 29 and 31 — As a workspace member, I want to save an ingredient with only a name and see errors beside the field that caused them, so that I can capture something quickly and fix mistakes without hunting.*
...
*Story 16 — As a workspace member, I want a warning when the name I'm typing resembles an existing one, so that I can reuse an entry instead of duplicating it.*
...
*Stories 3–13 — As a developer, I want the workspace stories expressed as failing tests before implementation, so that authorization is verified against the specification.*
...
*Story:* As a user, I want workspaces with members so that a coven or household can share one inventory.
...
*Story 12 — As a viewer, I want to read everything in a workspace but change nothing, so that I can be included without putting shared records at risk.*
...
*Story:* As an owner, I want the database itself to enforce workspace boundaries so that an application bug cannot leak my grimoire.
...
*Story 19 — As a workspace owner, I want the database to block cross-workspace access even if the application forgets to, so that one missed check in a service is not a data breach.*
...
*Story 19 — As a workspace owner, I want both outsiders and site admins refused access to my workspace, so that neither a stranger nor a curator can read what my coven records.*
...
*Story 3 — As a signed-in user, I want to create a workspace, so that my coven or household has a shared place to work.*
...
*Story 11 — As a workspace owner, I want to be blocked from removing the last owner, so that a workspace cannot be orphaned by one careless click.*
...
*Story 8 — As a member of several workspaces, I want to switch between them without losing my place, so that moving between my own notes and a coven's is quick.*
...
*Story:* As a user, I want a workspace-scoped layout so that every page below it knows which workspace it is in without guessing.
...
*Story:* As an operator, I want membership lookups batched so that a members page does not issue one query per row.
...
*Story 9 — As a workspace member, I want each member shown with their role and joining date, so that the membership list is readable at a glance.*
...
*Story 9 — As a workspace member, I want to see who is in the workspace and their roles, so that I know who can read what I write.*
...
*Story:* As an owner, I want to delete a workspace I no longer use so that abandoned covens do not clutter my switcher forever.
...
*Story 10 — As a workspace owner, I want to remove a member, and as a member I want to leave, so that membership reflects who is actually involved.*
...
*Story 10 — As the only owner of a workspace I no longer want, I want to be shown how to hand it over or close it, so that I am not left believing I am stuck in it forever.*
...
*Story 12 — As a workspace owner, I want viewers to be unable to change shared records, so that I can share our ingredient records without risking them.*
...
*Story 13 — As a workspace member, I want to see who last edited an ingredient and when, so that I know who to ask about a change.*
...
*Story:* As an owner, I want invitations stored safely so that a leaked database row cannot be redeemed.
...
*Story 4 — As a workspace owner, I want an invitation link shown once and stored only as a hash, so that a leaked database row cannot be redeemed.*
...
*Story 4 — As a workspace owner, I want to generate an invitation link with a chosen role, so that I control what a new member can do before they arrive.*
...
*Story 4 — As a workspace owner, I want to copy the invitation link and be told plainly that it will not be shown again, so that I do not lose it silently.*
...
*Story 6 — As an invited person, I want to accept an invitation and land in the workspace, so that joining takes one click and a sign-in.*
...
*Story 5 — As a workspace owner, I want to revoke a pending invitation, so that I can undo an invite sent in error.*
...
*Story 7 — As a workspace owner, I want expired, revoked and already-used links rejected, so that an old link in someone's inbox is not a way in.*
...
*Stories 14–16 — As a developer, I want the compendium browsing stories expressed as failing tests before implementation, so that done is measured against the specification.*
...
*Story 15 — As a workspace member, I want to create an ingredient local to my workspace when the compendium lacks it, so that my practice is not limited to someone else's list.*
...
*Story:* As a user who disagrees with an admin's correspondences, I want my own version to win in my workspace so that curation does not override my practice.
...
*Story 21 — As a workspace member, I want to search by name or folk name, so that I can find Devil's Shoestring without remembering it is honeysuckle root.*
...
*Story 14 — As a workspace member, I want to browse the compendium, so that I can add shared entries to my own ingredients.*
...
*Story:* As an operator, I want the compendium served from cache so that the most-read data on the site does not hit Postgres on every page.
...
*Story:* As a user, I want compendium edits to appear promptly so that the cache does not serve me an admin's outdated correspondences for an hour.
...
*Stories 15 and 34 — As a workspace member, I want my edits saved and reflected in the list immediately, so that I can trust what I am looking at.*
...
*Stories 28–34 — As a developer, I want the add and edit modal stories expressed as failing tests before implementation, so that the form's behaviour is specified rather than discovered.*
...
*Story 21 — As a workspace member, I want to type a name or folk name and see the list narrow, so that finding something is faster than scrolling.*
...
*Stories 22 and 30 — As a workspace member, I want to filter by several categories from grouped chips, so that I can narrow 52 categories on a phone without scrolling forever.*
...
*Story 22 — As a workspace member, I want to combine categories with 'any' as well as 'all', so that I can search broadly when nothing matches every term.*
...
*Story 27 — As a workspace member, I want to filter to only our own ingredients or only compendium ones, so that I can review what we have added ourselves.*
...
*Stories 23 and 53 — As a workspace member, I want stock and safety flagged on the card, so that I notice a toxic ingredient or an empty jar without opening it.*
...
*Story 17 — As a workspace member, I want compendium entries shown as read-only with a reason, so that I understand why I cannot edit rather than assuming it is broken.*
...
*Story 28 — As a workspace member, I want to open the add form from anywhere via the nav, so that recording something does not cost me my place.*
...
*Stories 32 and 33 — As a workspace member, I want the edit form pre-filled and a warning before discarding changes, so that I do not lose work by closing a dialog.*
...
*Story 14 — As a workspace member, I want a browsable compendium page, so that I can find shared entries and add them to my ingredients.*
...
*Story:* As a user, I want a page per ingredient so that correspondences have a permanent home I can link to.
...
*Stories 20–27 — As a developer, I want the workspace ingredient stories expressed as failing tests before implementation, so that stock behaviour is specified rather than discovered.*
...
*Story 20 — As a workspace member, I want our ingredients and quantities recorded, so that I can see everything we hold in one list.*
...
*Story 12 — As a workspace owner, I want members to change stock and viewers only to read it, so that stock records match who is responsible for them.*
...
*Story 14 — As a workspace member, I want to add a compendium entry to our ingredients, so that I do not re-type what is already described.*
...
*Story 24 — As a workspace member, I want quantities compared across units of the same kind, so that a recipe in teaspoons can be checked against a jar measured in millilitres.*
...
*Story 20 — As a workspace member, I want one page listing everything we hold, so that I do not have to look in two places.*
...
*Story 27 — As a workspace member, I want to filter my ingredients by where an entry came from, so that I can review what we added ourselves.*
...
*Story 23 — As a workspace member, I want low and empty stock flagged, so that I know what to replace before I start a working.*
...
*Story 24 — As a workspace member, I want to edit quantities from the row, so that updating stock after a working takes seconds.*
...
*Story 25 — As a workspace member, I want deletion to ask first and be recoverable, so that a mis-tap does not lose a record.*
...
*Story 26 — As a new workspace member, I want an empty ingredient list to prompt my first addition, so that I know what to do next.*
...
*Story 14 — As a workspace member, I want to add a compendium entry to our ingredients with a quantity, so that browsing leads directly to a stocked item.*
...
*Stories 47–56 — As a developer, I want the grimoire stories expressed as failing tests before implementation, so that spell behaviour is specified rather than discovered.*
...
*Stories 47 and 50 — As a workspace member, I want a spell recorded with its intent and its ingredients, so that I can repeat a working exactly.*
...
*Story:* As a workspace member, I want to keep a spell to myself, so that I can record a working that is nobody else's business without leaving the workspace.
...
*Story 48 — As a workspace member, I want to assign categories describing what a spell is meant to do, so that intent is recorded alongside contents.*
...
*Story 56 — As a workspace member including a viewer, I want to read every spell in the grimoire, so that shared knowledge is actually shared.*
...
*Story:* As a workspace member, I want a spell I shared to stay shared, so that nobody can quietly withdraw something the rest of us have been working from.
...
*Story 52 precondition — As a workspace member, I want the categories of a spell's ingredients gathered into one set, so that I can compare what is in the jar against what I intended.*
...
*Story 52 — As a workspace member, I want gaps shown in both directions between intent and ingredients, so that I can see what is missing and what I did not plan for.*
...
*Story 52 — As a workspace member, I want the comparison available from the API, so that it is computed once rather than reimplemented in each view.*
...
*Stories 47, 50 and 55 — As a workspace member, I want to create a spell, add ingredients and save it as a draft, so that I can build it over more than one sitting.*
...
*Story 56 — As a workspace member, I want to browse our spells, so that I can find and reuse past workings.*
...
*Story 47 — As a workspace member, I want to name a spell and state its intent, so that I know what it was for months later.*
...
*Story:* As a workspace member, I want to choose whether a spell is mine or ours while I build it, so that I decide before anyone has seen it rather than afterwards.
...
*Story 48 — As a workspace member, I want to tag a spell with intent categories, so that I can find it by purpose.*
...
*Stories 49 and 50 — As a workspace member, I want the same search controls as my ingredients page and to add ingredients with a quantity, so that building a spell uses tools I already know.*
...
*Story 51 — As a workspace member, I want to reorder ingredients, so that layering sequence is recorded as part of the recipe.*
...
*Story 52 — As a workspace member, I want to see where intent and contents disagree while I build, so that I can correct the jar rather than discover it later.*
...
*Story 53 — As a workspace member, I want a warning when an ingredient is toxic or unsafe to burn, so that I do not harm myself or someone I give it to.*
...
*Story 54 — As a workspace member, I want to see when I am adding something we have none of, so that I know what to gather before starting.*
...
*Story 55 — As a workspace member, I want to save a spell as a draft and mark it complete later, so that an unfinished spell is not mistaken for a finished one.*
...
*Story:* As a user, I want a saved spell to remain intact after I run out of something so that my record of a working is not damaged by stock changes.
...
*Story:* As a user, I want a printable recipe so that I can work from paper without a screen beside the jar.
...
*Story 18 — As a site admin, I want compendium curation to work through the real interface against a production build, so that the path users take is the path that is tested.*
...
*Story 14 — As a workspace member, I want adding a compendium entry to our ingredients to work end to end, so that the core daily loop is verified.*
...
*Stories 47–52 — As a workspace member, I want spell building and the category comparison to work end to end, so that the most complex screen is verified as a whole.*
...
*Stories 4 and 6 — As a workspace owner, I want inviting a member to work end to end, so that sharing behaves as promised.*
...
*Story 12 — As a viewer, I want my read-only access verified end to end, so that the role means what it says.*
...
*Story 19 — As a workspace owner, I want isolation from other workspaces verified end to end, so that privacy is proven rather than assumed.*
...
*Story 25 — As a workspace member, I want soft delete verified end to end, so that removal behaves the same way in the browser as in the service tests.*
...
*Story:* As a user relying on assistive technology, I want every page and modal to meet the standard so that no part of the app is closed to me.
...
*Story:* As a user returning after a quiet week, I want the first paint to be immediate so that Neon's resume is invisible rather than looking broken.
...
*Story:* As a user, I want failures to be legible so that an error is a message rather than a blank screen.
...
*Story:* As a developer, I want the gates proven before release so that the first post-launch PR is not the one that discovers they were misconfigured.
...
*Story:* As a future maintainer, I want each subsystem and component documented so that the design decisions survive the people who made them.
...
*Story:* As a product owner, I want every one of the 44 stories passing so that done is measured against the specification.
...
*Story:* As a developer, I want v1 released through the standard Gitflow path so that the first production deploy exercises the same process as every later one.
