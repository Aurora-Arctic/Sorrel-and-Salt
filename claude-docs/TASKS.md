...
| **6 — Auth surface**         | M2.1 · M2.4 · M2.5 · M2.6 · MB.55 · M2.7 · M2.9 · M2.10 · MB.12 · MB.54 · MB.56 · MB.57                                                                                                        | M2.8 defers to Wave 10. M2.10 needs M2.7's session helper and M0.30's Ladle build, both of which exist by the end of this wave. MB.12 closes the wave: "a real browser completes sign-in" is unmeetable before M2.6 builds the page to sign in on. **MB.55 lands as early in the wave as it can**: it carries two M2.6 defects that #156 merged without, one of which leaves the `playwright` leg red on every PR that touches app code. MB.12 is what exposed it — registering the provider credentials is the first time any environment had them — but it does not wait for MB.12 to close.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **8 — Compendium + admin**   | M4.5 · M4.7 · M4.7a · M4.8 · M8.2 · M8.5 · M8.8 · MB.11 · M5.1 · M5.2 · M5.3 · M5.4 · M5.9 · M5.10 · M5.10a · M5.5 · MB.36 · M5.6 · M5.6a · M5.6b · M5.7 · MB.52 · MB.53 · M5.8 · M8.6 · M8.7  | M5.5/M5.6 read and write through GraphQL, which is M8.5/M8.8 — so those move ahead of M5.5. M5.9/M5.10 build `IngredientForm`, which M5.5 consumes, so they precede it too. **M5.10a joins M5.9/M5.10 ahead of M5.5** because M5.5 consumes `IngredientForm`; **M5.6a follows M5.6 but precedes M5.7**, which gates it. **MB.52 precedes M5.8**, whose approval control acts on the user list rather than building one; MB.53 follows MB.52, since impersonation needs somewhere to pick a target.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
- `/admin/users` lists name, email, role, `canCreateWorkspace` and created-at, server-rendered through a service, sorted stably
...
**MB.54 — Collect an email address when a provider returns none** · 2h

_Story:_ As a new user signing in with a provider that doesn't share an email — a Discord account with no verified one, or a Facebook profile that withholds it under the permissions granted — I want a short prompt to supply one, so that a missing email doesn't turn into a dead-end sign-in failure.

Minted during M2.6, which re-scoped the OAuth roster to Google, Discord, Facebook and Microsoft. Discord returns an email only for an account with a verified one; Facebook can withhold it depending on scopes. Today (and after M2.6) that callback fails as `?error=email_not_found` and the sign-in page shows a readable-but-dead-end sentence (`src/lib/sign-in.ts`'s `ERROR_MESSAGES`). This task replaces the dead end with a short interstitial: collect an address, validate it, and complete the account creation with it.

The design question this task actually has to answer, not just the UI: Better Auth's callback redirect carries only a `?error=` code, no session and no reference back to which provider account the attempt belongs to — the OAuth exchange already happened and failed only at the "create the user" step. So the interstitial needs a way to hold that pending identity across the extra round trip (a short-lived signed token carrying the provider and its account id is the likely shape, not a database row — a partial `users` row would violate the same audit invariants a real sign-up respects). Scheduled after M2.7, whose session helper this most likely reuses for the post-completion sign-in.

_Acceptance criteria:_

- A provider callback with no email lands on an interstitial instead of the dead-end error
- The submitted address is validated before the account is created
- The resulting user is created with that address, through the same path as any other sign-up (admin bootstrap check, audit self-stamping)
- Declining or abandoning the interstitial leaves no partial `users` row
- Covered by a component test and at least one acceptance-level check
