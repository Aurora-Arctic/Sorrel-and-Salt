...
**Re-scoped by MB.24** in two places. The visibility clause reads the acting user through `app.current_user_id()` like every other policy, not through a second `current_setting` call — one definition, so a change to how identity is published cannot leave this policy reading the old way. And `spell_ingredients` and `spell_categories` reach their spell by FK: whatever this task does to `spells`, a private spell's _ingredients_ must not remain readable through the join tables, or the visibility rule holds for the spell and leaks its contents.

_Acceptance criteria:_

- Column added with a default of workspace and no nulls
- RLS policy admits a private spell only to its author, reading the user through `app.current_user_id()`
- A private spell's rows in `spell_ingredients` and `spell_categories` are refused to everyone but its author
- The policy is proved with the service check stubbed, as in M6.5, including that task's precondition assertions
...
| MB.24 | Decide the RLS role split and read-path identity                                             | Wave 3  | MB.25, MB.26 |
...
**MB.24 — Decide the RLS role split and read-path identity** · 2h
...
**MB.25 — Split the database roles so RLS applies to the app** · 2h

_Story:_ As an owner, I want the application to connect as a role that cannot bypass RLS, so that the database layer enforces workspace boundaries rather than merely declaring them.
...
**MB.26 — `withViewer`, the read-side identity wrapper** · 2h

_Story:_ As a developer, I want reads to carry the acting user into the database, so that an RLS policy can filter a read rather than erroring on a setting that was never published.
...
- A guard test modelled on `src/test/soft-delete-finder-guard.test.ts`
...
_15 tasks · 1 hour each, except MW.15 at 3 hours_
...
**MW.15 additionally carries the v1 close-out** that M11.15 held, which is why it is sized at 3h rather than 1h. It is not a sixteenth compression pass with a bigger diff — it is the pass that ends the mechanism, so the docs a v2 reader opens are short, true, and the only thing they have to open. Three parts, in this order, because verifying a doc you are about to rewrite wastes the verification, and retiring the archive before the live docs are settled hides what is missing:
...
Part 3 replaces the earlier plan to _fold_ the per-wave archives into a v1 archive with an index. Deleting the archive is only safe because of a rule every `MW.<n>` has already enforced — **nothing may end up living only in the archive** — and fourteen passes have each asserted it before this one asserts it again. So the deletion is the proof of that rule rather than a new risk: if something is lost, the rule was already broken and had been for some time. Keeping a directory nobody reads as insurance against passes that were supposed to make it unnecessary is the failure mode, not the safeguard, and git history keeps every archived file retrievable in any case — which is what makes this a compression decision rather than a destructive one. It can only happen here, at the last pass, because until then there is always another wave to feed the archive.

This deliberately supersedes CLAUDE.md's "Text leaving a live doc moves to `claude-docs/archive/` — never deleted", which is written for a project still in flight. The docs will have changed substantially by the time MW.15 runs, so scope it against the docs as they are then, not as they are today.
...
- Text leaving a live doc moves to `claude-docs/archive/`; an already-archived file is never edited (MW.1–MW.14 — MW.15 retires the archive instead)
