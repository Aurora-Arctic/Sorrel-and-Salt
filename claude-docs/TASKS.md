...
| MB.44 | Release to `main`, then drop the coverage excludes MB.42 made dead                           | Wave 4  | —            |
...
**MB.44 — Release to `main`, then drop the coverage excludes MB.42 made dead** · 1h

_Story:_ As a developer, I want the coverage number to come from the repo's own configuration rather than from two entries propping it up, so that what the gate measures is what the repo contains.

**The leftover MB.42 could not take with it.** `vitest.config.mts` excludes `src/**/*.test.{ts,tsx}` and `src/test/**` from coverage. Both paths have been empty since MB.41, and the entries survive only because `include` enumerates the _disk_ rather than the repo: CI's `/app` held every file the repo had deleted, uncovered, dragging the denominator down. Dropping them took CI from 92% to 78.54% with every test passing.

MB.42 closed that condition — `checkout-to-app` runs `git clean -fd` after its copy — and still could not remove them, because **the fix is not live where it lands**. Every caller references the action as `Aurora-Arctic/Sorrel-and-Salt/.github/actions/checkout-to-app@main`, so a job resolves whatever is on `main` at the moment it starts. A merge to `staging` does not reach that. Removing the entries in MB.42's own PR would have failed the 80% gate on that PR, against a container still carrying the leftovers.

So this is two steps, and the first is not a code change: **cut a release** (`/create-release` — `release/<version>` off `staging`, PR into `main`) so that the fixed action is what every job resolves, and then, on a PR after that release has _merged_, delete the two entries and remove the three places that currently explain why they stay. Opening the release PR is not enough; `@main` resolves at job start.

The removal is the cheap half. The release is the gate, and it is worth doing on its own terms — MB.42's fix protects nothing until it is on `main`, and neither does anything else that has accumulated on `staging`.

_Acceptance criteria:_

- A release branch merged to `main` carrying MB.42's `checkout-to-app`, and one container job observed on the first PR after it: the "Remove files the checkout no longer contains" step present in its log, and `checks / overlay` green
- `src/**/*.test.{ts,tsx}` and `src/test/**` gone from `vitest.config.mts`'s coverage `exclude`. `src/db/migrations/**`, `src/db/seed/**` and `*.stories.tsx` stay — each excludes files that really exist, for its own stated reason
- `npm run test:coverage` green in CI with the 80% thresholds unchanged, and the PR body names the reported number against the 92.03% it was before. **The point is that it does not move**: if the entries are dead they match nothing, so removing them changes nothing
- **If the number does move, the entries were not dead.** Stop rather than restoring them, and report what is still on disk in the container — that is a live MB.42 regression, not a coverage problem
- `vitest.config.mts`'s comment, `testing.md`'s Coverage paragraph and `ci.md`'s "one workaround is left standing on purpose" note are all **removed** rather than reworded: the condition they describe no longer exists, and a rewritten version of it is the kind of statement CLAUDE.md's "is this out of date?" test exists to catch
