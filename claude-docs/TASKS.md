...
| **4 — Seed and harness**     | M1.21 · M4.3 · M4.3a · M1.22 · MB.40 · M1.26 · M1.23 · MB.41 · M1.25 · M1.24 · M1.27 · M1.28 · MB.27 · MB.42                                                                                                                  | The payoff wave: retires all three workarounds. M4.3 is pulled ahead of M1.22, which consumes its 63 categories; **M4.3a is pulled ahead of M1.22 for the same reason**, since M1.22 also consumes its form vocabulary. **MB.40 sits after M1.22 and before M1.23**: it is DDL on `spell_ingredients`, empty and unqueried until Wave 13, so it lands as early as the schedule allows after it was minted — and ahead of M1.23, the first writer of that table, whose demo scenario seeds one custom row against the reshaped key. It is the repo's first contract migration, taken against zero rows. **MB.26 was on this row and is retired** (MB.29) — there is no read-side wrapper to land ahead of a caller that no longer exists. MB.27 is CI-only and depends on nothing here. **MB.41 sits after M1.23 and before M1.28**: it moves the whole suite into `tests/`, so it wants every Wave 4 test file that is going to exist already written — and it must precede M1.28, which creates `tests/acceptance/` and would otherwise create it beside a `src/` suite it does not match. **MB.42 closes the wave, beside MB.27 and for the same reason** — it is CI-only and depends on nothing here. Where in the wave it sits barely matters: the fix is a Dockerfile change, and `pr-gate.yml` builds the image content-addressed per PR, so it is live on its own PR and on every run after. It was missing from this row until MB.43's pass; the MB index had carried it as Wave 4 all along.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **5 — Authorization**        | MB.45 · MB.46 · MB.47 · M6.3 · M10.3 · M6.6 · MB.48 · MB.50 · MB.51                                                                                                                                                           | All seven workspace-scoped tables now exist, so M6.3's `Membership` sweep over the finders is complete rather than partial — the reason this wave waits on the schema block. **M6.4 and M6.5 were on this row and are retired** (MB.29); M6.6 now carries the whole burden of proving isolation, which is why its direct-id denial tests are per-entity rather than a sample.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **6 — Auth surface**         | M2.1 · M2.4 · M2.5 · M2.6 · MB.55 · M2.7 · M2.9 · MB.60 · M2.10 · MB.12 · MB.61 · MB.54 · MB.56 · MB.57                                                                                                                       | M2.8 defers to Wave 10. M2.10 needs M2.7's session helper and M0.30's Ladle build, both of which exist by the end of this wave. MB.12 closes the wave: "a real browser completes sign-in" is unmeetable before M2.6 builds the page to sign in on. **MB.55 lands as early in the wave as it can**: it carries two M2.6 defects that #156 merged without, one of which leaves the `playwright` leg red on every PR that touches app code. MB.12 is what exposed it — registering the provider credentials is the first time any environment had them — but it does not wait for MB.12 to close. **MB.60 lands before MB.12**, which sets the `ADMIN_BOOTSTRAP_EMAIL` it guards. **MB.61 precedes MB.54**: MB.54's typed address becomes one the user verifies, so MB.54 waits for MB.61's follow-ups rather than building an unverified path that would then be replaced.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| **8 — Compendium + admin**   | M4.5 · M4.7 · M4.7a · M4.8 · M8.2 · M8.5 · M8.8 · MB.11 · M5.1 · M5.2 · M5.3 · M5.4 · M5.9 · M5.10 · M5.10a · M5.5 · MB.36 · M5.6 · M5.6a · M5.6b · M5.7 · MB.52 · MB.53 · M5.8 · MB.58 · MB.59 · MB.62 · MB.63 · M8.6 · M8.7 | M5.5/M5.6 read and write through GraphQL, which is M8.5/M8.8 — so those move ahead of M5.5. M5.9/M5.10 build `IngredientForm`, which M5.5 consumes, so they precede it too. **M5.10a joins M5.9/M5.10 ahead of M5.5** because M5.5 consumes `IngredientForm`; **M5.6a follows M5.6 but precedes M5.7**, which gates it. **MB.52 precedes M5.8**, whose approval control acts on the user list rather than building one; MB.53 follows MB.52, since impersonation needs somewhere to pick a target. **MB.58 and MB.59 follow M5.8**: the grant control sits on the same row as M5.8's, and the ledger table precedes the service that writes it. **MB.62 and MB.63 follow MB.59**, since the pause switch acts on its grant.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **15 — Launch**              | M11.1 → M11.7 · M11.16 · M11.8 → M11.14 · MW.15                                                                                                                                                                               | M11.16 (the site's images) was minted on request during Wave 6 and lands before M11.8, whose axe sweep scans its alt text, and before M11.9 and M11.10, whose pages use what it sources. MW.15 closes the project, and is the one close-out pass sized at 3h — see the MW section.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
- M2.9 is a scoping task whose output is a decision doc and a follow-up task, not code. Its follow-ups are MB.58 (the ledger table), MB.59 (grant and revoke), MB.60 (the bootstrap promotion fixed) MB.61 (scoping first-party email verification, which re-scopes MB.54 and M7's delivery), and MB.62 and MB.63 (the pause switch), in [`design-decisions/m2.9-granting-admin.md`](design-decisions/m2.9-granting-admin.md). Schedule them before a second admin is actually needed.
...
**M2.5 — Configure the GitHub OAuth provider** · 1h · **reversed at M2.6**
...
**Reversed.** M2.6 re-scoped the provider roster to Google, Discord, Facebook and Microsoft and removed GitHub; this task's own criteria stand as a record of what merged and was later undone, per task IDs being immutable (the MB.19 shape — kept for provenance, not because it still describes the running app). The GitHub OAuth App itself and its Vercel environment variables are deleted manually, outside this repo.

**M2.6 — Build the sign-in page, on a re-scoped provider roster** · 2h

_Story 1 — As a new user, I want to sign in with an account I already have, so that I don't have to manage another password._

`/sign-in` with the roster's OAuth buttons, an error state for a failed callback, and a redirect to the intended destination after success. Component test with role and label queries only.

**Widened in place, by explicit instruction, rather than split into a separate re-scoping task first** (the sequencing this repo otherwise prefers — MB.28, MB.35): the provider roster became **Google, Discord, Facebook and Microsoft**, dropping **GitHub** (M2.5, reversed above). Apple was considered and dropped — its client secret is a JWT Apple caps at six months, expiring silently rather than failing loudly, on top of a paid Developer Program membership the other four don't need. DESIGN.md §2/§8/§10 (story 1's own text) were corrected in the same PR. `claude-docs/secrets.md` carries the registration walkthrough for the three unregistered providers; none of the four new OAuth applications exist yet, so the sign-in page ships with only Google live and the rest greyed out (`configuredProviders()`) until credentials land. **MB.54** was minted alongside this task: Discord and Facebook can both return a profile with no email, which this task only turns into a readable error rather than a recoverable one. MB.55 was minted during MB.12, when registering the four OAuth applications put real client ids in a local `.env.local` and M2.6's own accessibility spec — green in CI three times over — immediately failed on a genuine WCAG AA contrast violation. **The check was passing because the thing it checks was not on the page**: with no credentials configured, every provider button renders greyed and drops its brand colour, so CI scanned a page whose brand colours did not exist. It is MB.39's shape rather than MB.24's — not a constraint nobody re-derived, but a test whose green depended on an environment nobody had varied. It also carries M2.6's two leftover defects, since #156 merged before they could ride along.

_Acceptance criteria:_

- Every roster provider reachable by keyboard
...
The landing is **`/coven`** — its own protected route and `/sign-in`'s default `next`, decided by MB.57 ([`design-decisions/mb.57-post-sign-in-landing.md`](design-decisions/mb.57-post-sign-in-landing.md)); `/` is the public entry page and is not this task's. Story 2's acceptance test reads `src/app/coven/page.tsx`.
...
- Users awaiting approval are findable, so an admin can act without being sent an id — on MB.52's `/admin/users`, filtered to `canCreateWorkspace = false`, rather than a list this task builds for itself
...
- **Delivery is re-scoped to email** (M2.9, [`design-decisions/m2.9-granting-admin.md`](design-decisions/m2.9-granting-admin.md)). The link is mailed to the invited address and accepted only by an account whose verified email matches it; nothing is shown to copy. M7.1's schema, M7.2's token and hash, M7.6's revoke and M7.7's rejections stand as written. M7.3's returned URL, M7.4's copy dialog and M7.5's acceptance change, and MB.61's follow-ups rewrite those three entries and §7's `InvitationResult` before Wave 10 opens. Story 4 already reads the new way.
...
- M11.16 (images) lands before M11.8, M11.9 and M11.10: the sweep scans its alt text, and the skeleton and error pages use the illustrations it sources. It skips M11.15, a retired id.
...
_Story:_ As a product owner, I want every one of the 50 stories passing so that done is measured against the specification.

Run `make test-stories` and confirm all 50 pass. Any deferral is recorded explicitly rather than left silently failing.

_Acceptance criteria:_

- All 50 stories report pass
...
**M11.16 — License or create the site's images** · 3h

_Story:_ As the site owner, I want every image the site ships to be one we made or hold a licence for, and to be able to say which, so that launch does not put a picture on a public page whose terms nobody checked.

Minted on request during Wave 6. M11.16 rather than M11.15, which is a retired id.

The site ships no images today: no favicon or app icons, no social-share image, and every page is type on a surface. Before launch, decide what imagery the site needs and source each piece by licensing it or by making it. The likely set: favicon and app icons plus the web manifest; an Open Graph / social-share image; a mark for the front door (`/`, MB.57); and the illustrations the empty and error states want — M2.8's invite-only explanation, M5.4's not-authorized page, M11.9's skeletons, M11.10's 404/500 pages. The set is the task's first output, not its input: an inventory of which pages need what, agreed before anything is bought or drawn.

Sized at 3h rather than the usual 1–2h because sourcing is not coding: reading licence terms, commissioning or drawing, and exporting formats each take their own sitting, and an inventory-then-source pair is one decision rather than two tasks.

**Every image pulled in goes on one list, `claude-docs/images.md`:** the file; the page(s) that use it; where it came from — licensed from whom under which licence, commissioned from whom, or made in-house by whom and with what; the licence's terms in a clause (attribution required or not, modification allowed or not, commercial use allowed or not); the attribution wording if any; and where a copy of the licence text or the receipt is kept. Nothing generated by a model goes on a page without a licence position recorded on the same list. Licences must permit commercial use even though the Vercel Hobby plan is non-commercial (DESIGN.md §4): a licence that covers only today's plan is a relicensing task the day the site earns anything. Follows the M0.6 precedent for typefaces — chosen, licensed for the intended use, recorded.

**Two placeholders are already shipping and must go.** The masks behind the `Backdrop` component on every page are derived from photographs used without a licence, on the understanding that this task replaces them: `src/components/Backdrop/salt-spoon.webp` from `salt-spoon-slate.jpg` on the Salt Association's website (`saltassociation.co.uk`, marked "© Salt Association"), and `src/components/Backdrop/sorrel-plate.webp` from a Feast Magazine editorial image (`feastmagazine.com`, served from `bloximages.newyork1.vip.townnews.com`). Each replacement is a photograph of the same subject on a dark plain ground — a bowl and a spoon of coarse salt, top-down and right-heavy, for the bottom-right corner; sorrel leaves on a plate, left-heavy, for the top-left — either licensed or shot in-house, run through `scripts/backdrop-masks.mjs` ([`components/backdrop.md`](components/backdrop.md), "The image is tone and shape"), and recorded in the ledger. The placeholder files are then deleted, and the ledger notes that they existed and when they left.

_Acceptance criteria:_

- An inventory names each image the site needs and the page that needs it, agreed before sourcing begins
- Every image shipped is licensed or created, and appears in `claude-docs/images.md` with its source, its licence and terms, attribution wording where required, and where the licence text or receipt is kept
- No image file in the repo is missing from that list, asserted by a guard test in `tests/guards/` that walks the shipped image files against it
- Every licence permits commercial use, modification and web distribution
- Attribution a licence requires is rendered where the licence says it must be
- Favicon, app icons and the web manifest exist and validate; a social-share image renders at the standard size and is set in the root layout's metadata
- Every image is served through `next/image` or as inline SVG, carries alt text or is marked decorative, and passes M11.8's axe sweep
- Raster sources are committed at the size the page needs, not the original, and the repo's size change is recorded in the PR body
- Both placeholders behind `Backdrop` — the Salt Association salt bowl and the Feast Magazine sorrel plate — are replaced by licensed or in-house photographs of the same subjects, given the same mask treatment by `scripts/backdrop-masks.mjs`, and deleted from the repo; the ledger records that they were there

Lands before M11.8, M11.9 and M11.10. Nothing in it depends on the wave, so it can be pulled earlier if the front door wants a mark sooner.
...
| MB.54 | Collect an email address when a provider returns none                                          | Wave 6  | M2.6, M2.7   |
...
| MB.61 | Scope first-party email verification and email invitations                                     | Wave 6  | MB.54, M7.3  |
| MB.62 | `site_settings` schema, with the admin-role-changes pause (schema)                             | Wave 8  | MB.63        |
| MB.63 | Pause admin role changes                                                                       | Wave 8  | —            |
...
**The fix is no source layer in either image, and a guard that keeps it out.** `Dockerfile.node` and `Dockerfile.e2e` copy in the two manifests, run `npm ci`, and stop — `node_modules` is all the image contributes to `/app`, so the checkout is the only source a job ever sees and a file the branch deleted has nothing to survive _as_. `checkout-to-app` stays checkout + `cp -a`. `tests/guards/image-source-layer.test.ts` parses every `COPY`/`ADD` in both Dockerfiles and checks each source against a per-file allowlist (the manifests, plus `Docker/playwright-entrypoint.sh` for `Dockerfile.e2e`'s `headed` stage, copied alone so a bare `docker run` of that stage still works) — an allowlist rather than a `.` denylist, because `COPY src src` is as much a source layer as `COPY . .` and a new COPY is a decision, not a convenience.

**The clean-step approach was built first on this PR and replaced.** PR #133's first commits fixed it the other way round: `git clean -fd` after the copy, then `git config --global --add safe.directory /app` when that failed with "dubious ownership" (`/app` is chowned to `node`, jobs run `--user root`), a `checks / overlay` job to manufacture the condition and prove the clean ran, `make act-overlay`, and MB.44 minted to remove the coverage excludes after a release. It was reversed for three reasons. The source layer was **shadowed everywhere but CI** — every compose service and the devcontainer bind-mount `..:/app` over it — so CI was its only reader, and there it was the bug; deleting the layer removes the condition rather than cleaning up after it. That is the sweep-task rule's tell: the clean step made stale files _absent_, no source layer makes them _impossible_, and nothing runs git in `/app` at all. And the fix is **live on its own PR**: `pr-gate.yml` calls `build-image.yml` with a tag hashed from `Dockerfile.node` and `package-lock.json`, so the PR builds and runs its own image — where an edit to the action is resolved at `@main` by every caller and takes hold only after a release. "Rebuild the image more often" was considered and rejected: it means hashing the source, so every push pays the `npm ci` the content-addressed tag exists to skip, to refresh a layer nobody reads. The reversal is recorded in [`design-decisions/mb.42-no-source-layer.md`](design-decisions/mb.42-no-source-layer.md).

**The coverage excludes go here too.** `vitest.config.mts` excluded `src/**/*.test.{ts,tsx}` and `src/test/**`, paths MB.41 emptied and the overlay kept alive — `include` enumerates the disk rather than the repo, and CI's container held every test file the repo had deleted; dropping them took CI from 92% to 78.54% with every test passing. With the image carrying no source they match nothing, and removing them in this PR is the proof: the number does not move. What this does **not** change: `/app` is still `node`-owned and vitest still runs as root, so the three guards that run `git -c safe.directory=*` (`test-location`, `lint-db-client-boundary`, `slug-rule`) stay as they are. Only the action stops running git in `/app`.
...
**The audit stamps the impersonated user, and says so twice.** `created_by`/`updated_by` record whoever the session is acting as — stamping the admin instead would mean impersonation no longer reproduces what the user would see, which is the whole point. So that the acting admin is not lost with the session row, `withAudit` publishes `app.impersonated_by` beside `app.current_user_id`, from the session and never a request body, through the same `set_config(..., true)`. It has no reader in v1 and is published anyway for exactly rule 3's reason: it is what makes the v2 history trigger one migration rather than a re-audit of every write path.
...
Minted during M2.6 as "collect an email address when a provider returns none": Discord returns an email only for an account with a verified one, Facebook can withhold it under the permissions granted, and the callback then fails as `?error=email_not_found`, a readable but dead-end sentence in `src/lib/sign-in.ts`. **Re-scoped by M2.9 to follow MB.61's follow-ups** ([`design-decisions/m2.9-granting-admin.md`](design-decisions/m2.9-granting-admin.md)). An address a user types is worth nothing until it is verified, and a typed-but-unverified address is also the easy way to squat someone else's: an unverified account holding an address blocks the real owner's verified sign-in, because Better Auth refuses to link a provider to an unverified row. So the interstitial does not create an account on a bare string. It shows the provider's address if there is one, lets the user change it, and the account's email takes effect once MB.61's verification confirms it. The same flow, reached from the account later, is how a user changes their email (story 59), which is why there is no separate change-email task: Better Auth's `changeEmail` stays pinned off (MB.60) and this flow replaces it.

The design question the original entry raised still holds: Better Auth's callback redirect carries only a `?error=` code, with no session and no reference back to which provider account the attempt belongs to, so the interstitial needs a way to hold that pending identity across the extra round trip. MB.61's follow-ups decide whether verification is required before the account exists or the account exists unverified with limits, and this task builds whichever they choose.

_Acceptance criteria:_

- A provider callback with no email lands on an interstitial, prefilled with nothing, instead of the dead-end error; one with an email is prefilled with it and still editable
- The address takes effect only after MB.61's verification confirms it; an unverified one never becomes `users.email` on a live row that another verified sign-in could be blocked by
- An address already held by a live account is refused with a message, asserted with the other account present
- The bootstrap address is refused outright
- The resulting user is created through the same path as any other sign-up (audit self-stamping), and no partial `users` row survives an abandoned interstitial
- A signed-in user can reach the same flow to change their email, with the same rules
- Covered by a component test with role and label queries, and stories 58 and 59's acceptance tests pass
...
A `setUserRole` service sits behind an admin-only GraphQL mutation, writes through `withAudit`, and appends a ledger row in the same transaction. Any admin may grant admin to any live user, verified or not: what the granting admin is trusting is the OAuth account behind the row, which MB.52's list shows, and confirming who someone is before granting is the admin's responsibility. A grant also sets `canCreateWorkspace`, so the row states what the person may do without a reader knowing that admins bypass the flag; a revoke leaves it. Any admin may revoke any other admin, including themselves, **except the primary admin**: the live admin whose email matches `ADMIN_BOOTSTRAP_EMAIL`, read from the variable at the time of each check. No one can revoke the primary admin, and it can't revoke itself. **A count guards the gap.** Between the variable changing and the new address's first qualifying sign-in nobody is protected, so the service also refuses any revoke that would leave zero live admins, locking the live admin rows `for update` before counting so two admins revoking each other at once cannot both succeed. With the primary admin in place the count is never reached. MB.60's sign-in promotion calls this same service with the freshly authenticated user's own session, so the promotion's `bootstrap` row is written through `withAudit` like every other, and the `standard` seed writes one for fixture E.
...
- A primary-admin promotion at sign-in produces a `bootstrap` ledger row through `withAudit`, and so does fixture E in the `standard` seed
- Stories 60 and 61's acceptance tests pass
- The mutation carries M5.7's Pothos admin scope as well as the service check
- `auth.md` documents granting, revoking, the primary admin's protection, the count fallback, how to change the primary admin (set the variable, redeploy, then a Google or Discord sign-in by the new address) and the ledger. It also records that any future user-deletion path must refuse the primary admin
...
**Only Google and Discord qualify.** Google marks an email verified only for a domain its owner has proved to Google, and Discord only for an address it has mailed a code to. Facebook never reports an address verified in Better Auth's mapping. Microsoft is excluded on purpose: with `tenantId: 'common'`, an attacker's own Entra tenant can issue an id token carrying any `email`, and Better Auth reads `email_verified` and the `verified_*_email` claims off that token. Those claims are meant to be Microsoft's own, but this is the surface the 2023 "nOAuth" attack used, and the safe default is not to rely on it. MB.61's first-party verification is what would let the other providers in.

**Promotion moves from account creation to sign-in, and writes through `withAudit`.** Better Auth's after-callback hook has `ctx.context.newSession.user` loaded, so the check costs one comparison per sign-in and no query. The decision uses the provider's fresh profile at the callback, never the stored `users.emailVerified`, so nothing that later sets that column can widen who qualifies. At that point the user is authenticated, so the hook builds an ordinary `Session` and calls the role service (a stub until MB.59) inside `withAudit`, stamped as the user. Rule 3's two identity bootstraps stay two: the create-time promotion that used one of them goes away. The session-create hook receives only a `userId`, so putting the check there would cost a lookup on every sign-in. Moving promotion to sign-in is what makes changing the variable work: the new address is promoted at its next qualifying sign-in even if the account already exists, and the previous primary admin keeps `role: 'admin'` and simply stops being protected (MB.59). A non-qualifying match signs in as an ordinary user, and the reason goes to the server log with the user id, not the address, since a sign-in screen should not reveal that an address is special.

**The squat is left open until MB.61's follow-ups, deliberately.** Refusing to create an account holding the address unverified, and mapping the `account_not_linked` code a refused link redirects to, were both in this task's first draft. They come out because first-party verification lands before launch and settles squatting for every address rather than one, and MB.54's typed-address form, the easy way to squat, now follows it. The hole that stays until then: an unverified Discord or Facebook sign-up carrying the address, made before the owner's first sign-in or after a database reset, blocks the owner's verified sign-in with a generic error. Pre-launch the site has no such users.
...
**Scoping task in M2.9's own shape: a decision doc and follow-up tasks, no code.** Today an address is verified only when a provider says so, and only Google and Discord say so in a way that can be trusted (MB.60). Mailing a link the user follows would let us vouch for the address instead, and that simplifies several things at once ([`design-decisions/m2.9-granting-admin.md`](design-decisions/m2.9-granting-admin.md), "Email verification and changing an email"): the primary admin could sign in through any provider; an account created with an unverified address, including one typed into MB.54's form, could prove it instead of blocking the owner's sign-in; a user can set or change their email (MB.54); workspace invitations go by email and are accepted only by the verified holder of the address, which is the decision already taken for M7 and story 4; and an admin invitation by email, M2.9's option B, is decided and waits only on this: the link is mailed to the named address and accepted by a signed-in account whose verified email matches, so acceptance is bound to the address rather than to whoever holds it (story 62).

**It needs email delivery, which v1 has none of.** Several providers have a free tier sized well past what this site would send, a few verification mails a week, and Resend is on the Vercel Marketplace. The discovery compares those free tiers against the site's real volume and against what each requires (a sending domain and its DNS records, a sandbox limited to verified recipients, or neither), and settles which one, if any, is worth the dependency. It then checks Better Auth's own `emailVerification` and `sendVerificationEmail` hooks against the rules here: the mail is sent from `/api/auth/*`, which is the one transport exception rule 1 already allows, the token must be stored hashed like an invitation's, and the verification write must be audited or argued as a third identity bootstrap, not just done. Until the follow-ups land, MB.60's rules stand as written.
