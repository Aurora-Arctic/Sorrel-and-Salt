...
//
// `categories` (M4.3) and `forms` (M4.3a) are the two arguments it takes, and
// neither is a scenario: DESIGN.md §6's category vocabulary and §5's form
// vocabulary are reference data the real site needs as much as a fixture does,
// so each seeds on its own, without the users a scenario would bring. They are
// what migrate.yml runs against staging and production, after applying
// migrations and in the same job. They live in this script rather than one of
// their own because the import below is a database-client exemption, and that
// set of four is pinned by test (claude-docs/db.md, "Who may import the
// client") — a fifth is a decision, and "the seed needed a second entrypoint"
// is not one worth making.
//
...
try {
  if (target === 'categories') {
    ...
    throw new Error(
      `Unknown seed target "${target}". Pass "categories" or "forms", or nothing for minimal.`,
    );
  }
} finally {
