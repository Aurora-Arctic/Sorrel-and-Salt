...
/** Everywhere the query-builder ban applies — application code. */
const RESTRICTED = ['src/services', 'src/graphql', 'src/app', 'src/components', 'src/lib', 'e2e'];

/** The database layer, which builds queries for a living. */
const EXEMPT = ['src/db', 'src/db/schema', 'scripts'];
...
// Rule 1 — every relative shape an importer in this repo can reach
// connection.ts by: a sibling inside src/db, a module one or two directories
// up inside src, and the extension-carrying specifier scripts/ must use.
...
afterAll(() => {
  for (const directory of [...RESTRICTED, ...EXEMPT]) {
    rmSync(join(repoRoot, directory, PROBE), { recursive: true, force: true });
  }
});
