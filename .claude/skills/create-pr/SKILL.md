...
7. **Gather context for the summary, and identify the Asana task.**
   ...
   - Find the task: `asana_search_tasks` with `projects.any` = `1218257926462425` and `text` = the task ID, then match a result whose `Task ID` custom field equals it exactly (the text search is fuzzy — don't rely on it alone). If zero or more than one task matches, tell the user and skip Asana for the rest of this skill. Note the matched task's `gid` and `permalink_url` — step 8 links the latter, step 10 reuses both without searching again.
