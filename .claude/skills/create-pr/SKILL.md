...
7. **Gather context for the summary, and identify the Asana task.**
   ...
   - Identify the Asana task, if any, now — step 8 links it in the PR body. Try the source branch name first: a `feature/m0.29-...` or `hotfix/m3.8-...` slug carries the task id as its leading `m<major>.<minor>` segment → `M0.29`. If the branch name has no such segment, ask the user for the Asana `Task ID` in plain chat (free-text, no options). If they don't have one, skip Asana entirely for the rest of this skill — no link in step 8's body, nothing to do in step 10.
   - Find the task by the two-call lookup in [`CLAUDE.md`](../../../CLAUDE.md)'s "Asana task tracking" section: `asana_get_tasks` with `project` = `1218814916390986` returns the wave cards and the top-level `MB.*` tasks; if the id isn't among them, `asana_get_task` on the wave card whose `notes` name it, with `opt_fields=subtasks.name,subtasks.gid,subtasks.completed`, and match the subtask's id segment exactly — strip the marker, then compare the text up to the `—` that follows it, because thirty of the board's ids are a strict prefix of another (`M2.1` and `M2.10`, `M4.1` and `M4.1a`). There is no text search — `asana_search_tasks` is premium and returns `payment_required`.
   - If zero or more than one task matches, tell the user and skip Asana for the rest of this skill. Note the matched task's `gid`, `name` and `permalink_url` — step 8 links the latter, step 10 reuses all three without looking up again.

8. **Draft the PR.**
   - Title: short, under ~70 characters, imperative mood.
   - Body: if step 7 identified an Asana task, start with a single `Asana task: <permalink_url>` line followed by a blank line — one line, not a section, so it doesn't crowd out what follows. Then a `## Summary` section (1-4 bullet points on _what_ changed and _why_, based on step 7's diff/log — not a restatement of commit messages) and a `## Test plan` section (checklist of how this was/should be verified — reference this repo's actual checks where relevant, per what was actually touched: `npm run pre-commit` today, plus `npm run test:coverage`, `make test-stories`, or the Playwright e2e run once the M1+ tasks wire them up. Only name a check that exists — this repo is pre-scaffold, so confirm a script is real before citing it).
...
10. **Move the Asana task to `In Review` and post the PR link.**
    ...
    - When step 9 **created** a new PR: in this same turn, `asana_create_task_story` with a comment carrying the PR URL, then `asana_update_task` with `name` set to step 7's name with its marker replaced by `◔ ` — strip a leading `▶ ` first, and leave the id and title untouched. For a `hotfix/*` branch, list every PR opened (`main`, `staging`, and the optional `release/*`) in the one comment, and set `◔ ` once — see [reference-hotfix.md](reference-hotfix.md).
    - When step 9 only **updated** an already-open PR: the task is already `In Review` — just add an `asana_create_task_story` comment noting the PR was updated, and leave the name alone.
    - Status moves one direction only: never tick the completed checkbox here (that happens on merge), and never move a task backward. If the name already starts with `◔ `, or the task is ticked complete, don't rename it — just add the comment.
...
- Step 10's project GID, status markers and lookup are mirrored from [`CLAUDE.md`](../../../CLAUDE.md)'s "Asana task tracking" section — that section is the source of truth if they ever drift. The workspace is on Asana's free plan, so status is a marker on the task **name**, not a custom field. Setting it the same turn the PR opens is a hard rule there ("A status left stale is worse than no status"), not a nicety.
